...
@Component({
  selector: 'app-overview-annonce',
  templateUrl: './overview.component.html',
  styleUrls: ['./overview.component.scss'],
})
export class OverviewComponent {
  data: { pagination: PaginationInfo; results: Annonce[] };
  ...
  constructor(
    ...
    private fetchAnnoncesGQL: FetchAnnoncesGQL,
    public dialog: MatDialog,
    private fb: FormBuilder,
    private snackBarService: SnackBarService,
    private deleteAnnonceGQL: DeleteAnnonceGQL,
    ...
    this.searchTerms
      .pipe(
        ...
        tap((term: string) => this.getAnnonces())
      )
      .subscribe((r) => {});
  }
  ...
  getAnnonces(useCache=true) {
    const annonceFilter = this.getFilters();
    const queryFilter = {
      limit: this.pageSize,
      page: this.currentPage,
      search: this.filterForm?.value?.search || null,
    };
    this.fetchAnnoncesGQL
      .fetch({ queryFilter, annonceFilter }, { fetchPolicy: useCache ? 'cache-first': 'no-cache' })
      .subscribe((result) => {
        this.data = result.data.fetchAnnonces as any;
  ...
  handleDeleteAnnonce(id: string) {
    this.openDialogDelete(id);
  }

  handlePublishAnnonce(id: string) {
    this.openDialogPublish(id);
  }

  handleUnPublishAnnonce(id: string) {
  ...
  openDialogDelete(id: string): void {
    ...
    dialogRef.afterClosed().subscribe((resp) => {
      if (resp) {
        this.deleteAnnonceGQL.mutate({ annonceId: id }).subscribe((result) => {
          if(result.data.deleteAnnonce) {
            this.getAnnonces(false);
          }
        });
      }
  ...
  openDialogPublish(id: string): void {
    ...
    dialogRef.afterClosed().subscribe((resp) => {
      if (resp) {
        this.publishAnnonceGQL.mutate({ annonceId: id }).subscribe(
          (result) => {
  ...
  openDialogUnPublish(id: string): void {
    ...
    dialogRef.afterClosed().subscribe((resp) => {
      if (resp) {
        this.unPublishAnnonceGQL.mutate({ annonceId: id }).subscribe(
          (result) => {
